...
export async function fetchRechargeHistory(
  ...
  const response = await apiClient.get<HistoryResponse<RechargeRecord>>('/creditos/recargas', {
    params,
  })
...
export async function fetchMealHistory(
  ...
  const response = await apiClient.get<HistoryResponse<MealRecord>>('/creditos/refeicoes', {
    params,
  })
  return response.data
}
